...
export const loginUser = async (credentials) => {
    ...
    const data = await getPromise('http://localhost:8080/login', option).catch(() => {
        return statusError;
    });
...
export const signupUser = async (credentials) => {
    ...
    const data = await getPromise('http://localhost:8080/register', option).catch(() => {
        return statusError;
    });
...
export const logoutUser = async (credentials) => {
    ...
    const data = await getPromise('http://localhost:8080/logout', option).catch(() => {
        return statusError;
    });
